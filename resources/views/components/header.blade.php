...
<header class="absolute top-8 left-8 z-20">
    ...
    <nav class="mt-3 opacity-0 pointer-events-none transition-all duration-500 peer-checked:opacity-100 peer-checked:pointer-events-auto">
        <ul class="flex flex-col p-0 space-y-1">
            ...
            <a href="{{ route('about') }}" class="text-gray-500 text-base no-underline transition-all duration-300 hover:text-black hover:translate-x-1 inline-block">About</a>
            {{ $slot }}
            @if(Auth::check())
...
@elseif($type === 'burger' && $side === true)
<header class="hidden md:block absolute top-8 left-8 z-20">
    <input type="checkbox" id="menu_checkbox_desktop" class="hidden peer">
    <label for="menu_checkbox_desktop" class="block w-8 h-8 cursor-pointer transition-transform duration-400 peer-checked:-rotate-90 [&:has(~input:checked)_.bar-dot]:w-1 [&:has(~input:checked)_.bar-dot]:ml-3.5 [&:has(~input:checked)_.bar-dot]:mb-0.5">
        <div class="bar-dot h-[3px] w-6 bg-gray-900 mb-1 rounded-[3px] transition-all duration-300"></div>
        <div class="bar-dot h-[3px] w-[18px] bg-gray-900 mb-1 rounded-[3px] transition-all duration-300"></div>
        <div class="bar-dot h-[3px] w-8 bg-gray-900 mb-0 rounded-[3px] transition-all duration-300"></div>
    </label>
...
<header class="fixed top-0 left-0 w-full h-16 bg-white border-b border-gray-200 flex items-center justify-between z-40 px-4 md:hidden">
    <div class="flex-1 flex items-center overflow-x-auto no-scrollbar">
        <a href="{{ route('home') }}" class="block px-3 py-2 text-gray-500 text-base no-underline transition-all duration-300 hover:text-black hover:translate-x-1">Home</a>
        <a href="{{ route('dashboard') }}" class="block px-3 py-2 text-gray-500 text-base no-underline transition-all duration-300 hover:text-black hover:translate-x-1">Dashboard</a>
        {{ $slot }}
    </div>
...
<style>
    ...
    #menu_checkbox_desktop:checked+label .bar-dot {
        width: 4px;
        margin-left: 14px;
        margin-bottom: 2px;
    }

    #menu_checkbox_mobile:checked+label .bar-dot {
        width: 4px;
        margin-left: 14px;
        margin-bottom: 2px;
    }
